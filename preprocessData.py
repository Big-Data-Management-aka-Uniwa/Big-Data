...
class PreprocessData:
    ...
    def __init__(self):
        ...
        # [Step 5] Keep only the data of Police killing in 2015 and in 2016 
        ...
        # [Step 6] Merge the 2 Police Killing datasets 
        self.PoliceKillingFinal = pd.concat([self.PoliceKillingFinal2015, self.PoliceKillingFinal2016], ignore_index=True)
        
        # [Step 7] Calculate the Poverty Average by state (PovertyUS_Avg_2015_2016.csv) 
        self.poverty_averages()

        # [Step 8] Calculate the Killings Average by state (PoliceKillingUs_Avg_2015_2016.csv)  
        self.killing_averages()
        
        # [Step 9] Print in the console the Processed Police Killing dataset
        print(self.PoliceKillingFinal)

        # [Step 10] Store the Processed Police Killing dataset in a .csv file
    ...
    def reduce_data_of_kills(self, df):
        # [Step 5.1] Groups data for every state
        grouped = df.groupby('state')  

        # [Step 5.2] Finds most frequent row for every state using most_frequent function
        summary = grouped.agg({ 
        ...
        # [Step 5.3] Rename the 'id' column to 'count'
        summary = summary.rename(columns={'id': 'count'}) 
        
        # [Step 5.4] Reset index to make 'state' a column
        summary = summary.reset_index()

        # [Step 5.5] Calculate total kills
        total_kills = df.shape[0] # get x dimension
        summary['percentage'] = (summary['count'] / total_kills) * 100
        
        return summary
    
    # [Step 5.2.1] Finds the most frequent value
    @staticmethod
    ...
    def poverty_averages(self):
        # [Step 7.1] Filter data for the years 2015 and 2016
        ...
        # [Step 7.2] Merge the two datasets on 'ID'
        merged_data = pd.merge(poverty_2015, poverty_2016, on='ID', suffixes=('_2015', '_2016'))
       
        # [Step 7.3] Calculate average values for the required columns
        ...
        # [Step 7.4] Select and rename the columns
        ...
        # [Step 7.5] Save the final poverty dataset to a CSV file
